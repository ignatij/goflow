...
func main() {
	...
	wfService.RegisterTask("sample input", service.WrapTaskFunc(func(args ...service.TaskResult) (service.TaskResult, error) {
		return "test", nil
	}), nil)

	wfService.RegisterTask("capitalize", service.WrapTaskFunc(func(args ...service.TaskResult) (service.TaskResult, error) {
		input := "goflow"
		if len(args) > 0 {
